...
- [x] 1. パッケージ取込と依存解決（Foundation）
- [x] 1.1 manifest.json に mocap-vmc / FacialControl 4 件を git+ssh で登録し VContainer を解決する
...
- [x] 2. AMFI 新規パッケージ雛形の作成（Foundation）
- [x] 2.1 AMFI パッケージの package.json と asmdef 一式を作成する（facial 非参照構成）
...
- [x] 2.2 Facial.asmdef を defineConstraints で Phase 1 未コンパイル化する
...
- [x] 3. AvatarCatalog データ層（Foundation）
- [x] 3.1 AvatarCatalog SO とエントリ型を作成し OnValidate バリデーションを実装する
...
- [x] 4. Addressables 非依存のアバター解決とスキーマ（Core）
- [x] 4.1 (P) CatalogAvatarKeyResolver を実装する
  ...
  - _Depends: 3.1_
- [x] 4.2 (P) InMemoryAvatarSchemaProvider を実装する
...
- [x] 5. VMC モーキャップ設定の配線（Core）
- [x] 5.1 (P) VmcMoCapSourceConfigFactory を実装する
...
- [x] 5.2* (P) MoCapRegistryProbe（任意）で登録済み typeId 一覧をログ出力する
...
- [x] 6. 覗き窓と全身モーション適用ループ（Core, rac-main-output-adapter への最小追加）
- [x] 6.1 RacMainOutputAdapterBootstrapper に SlotManager 公開プロパティを追加する
...
- [x] 6.2 SlotMotionDriver を実装する（LateUpdate 駆動ループ）
...
- [x] 7. AMFI Composition Root と IntegratedDemo 統合（Integration）
- [x] 7.1 AmfiCompositionRoot を実装する（Bootstrapper 生成・OverrideServices・Initialize・Driver 配線）
...
- [x] 7.2 IntegratedDemo に AMFI と既存 Host の排他起動分岐を実装する
...
- [x] 9. FacialControllerAttacher の実装（Core, Facial.asmdef）
- [x] 9.1 Scripting Define AMFI_FACIAL を有効化し Facial asmdef をコンパイル対象にする
  ...
  - _Requirements: 7.3_
  - _Depends: 2.2, 8.2_
- [x] 9.2 FacialControllerAttacher を実装する（実行時 Add + Profile + Initialize、IsInitialized ガード）
...
- [x] 10. Composition Root への Attacher 接続（Integration）
- [x] 10.1 AmfiCompositionRoot に Facial 有効化分岐と Attacher 配線を追加する
...
- [x] 12. README の整備（Integration）
- [x] 12.1 AMFI のセットアップ手順を README に記載する
